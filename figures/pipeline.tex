...
\begin{tikzpicture}[x=1.7cm, y=1cm, yscale = 1.0,
 ...
 %    \node[] at (-0.4, -6) {$\mathcal{N}$};
    ...
    \node[layer, minimum height = 2.65cm, draw = black, visible on = <2->] (input) at (0, 0) {};
    \begin{scope}[shift = {(0,-1)}]
        \node[minimum height = 0.5375cm, visible on = <2->] (s) at (0, 2.0375) {\small $\mathbf{s}$};
        \node[minimum height = 0.5375cm, visible on = <2->] (t) at (0, 1.5) {\small $\mathbf{t}$};
        \node[minimum height = 0.5375cm, visible on = <2->] (n) at (0, 0.95) {\small $\mathbf{n}$};
        \node[minimum height = 1.0375cm, visible on = <2->] (su) at (0, 0.19) {\small $\mathbf{SU}$};
    \end{scope}
    \draw[-, dashed, visible on = <2->] (s.south west) -- (s.south east);
    \draw[-, dashed, visible on = <2->] (t.south west) -- (t.south east);
    \draw[-, dashed, visible on = <2->] (n.south west) -- (n.south east);
