...
\begin{tikzpicture}[x=1.7cm, y=1cm, yscale = 1.0,
    ...
    \node[align = center, anchor = west, visible on = <1->] at (0.1,-0.6) {\large Processing Pipeline};

    \begin{scope}[xshift = 5.08cm, yshift = -3.2cm]

    \node[align = center, visible on = <2->] (demand) at (-2.25, 0) {demand and \\ network info};
 ...
 %    \node[] at (-0.4, -6) {$\mathcal{N}$};
    ...
    \begin{scope}[x=0.3cm, y= 0.5cm, shift = {(1.82cm, 1.44cm)}, visible on = <3->]
