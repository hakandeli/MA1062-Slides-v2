...
\begin{tikzpicture}
    ...
    \node[rectangle, fill = gray!40, minimum width = 14.7cm, minimum height = 6.5cm, rounded corners = 5pt, anchor = north west, visible on = <1->] at (0,0) {};
    ...
    \node[rectangle, fill = white, minimum width = 8cm, minimum height = 5cm, rounded corners = 5pt, anchor = north west, visible on = <5->] at (6.2,-0.8) {};

    %text
    \foreach \heur/\v in {  FF/7,
                            LF/8,
                            BFF/9,
                            BFL/10,
                            WFF/11,
    ...
    \node[anchor = center, align = center, visible on = <3->] at (3.1, -6.15) {K-shortest paths (KSP)};
    \node[anchor = center, align = center, visible on = <5->] at (10.2, -6.15) {Heuristic slot selection};
    ...
    \begin{scope}[x=0.9cm, y= 0.6cm, shift={(-0.2cm,-1cm)}]
        \foreach \x/\y/\lbl/\i in { 1.5/-4/{1.}/0, 2/-4/A/1, 3/-4/B/2, 4/-4/F/3, 5/-4/E/4,%
                                    ...
                                    \node[visible on = <4->] (n\i) at (\x, \y) {\small \lbl};
                                }
    \end{scope}
    \foreach \i/\j in { 1/2, 2/3, 3/4, 
                        ...
                        \draw[->, > = Stealth, line width = 0.2pt, visible on = <4->] (n\i) -- (n\j);
    ...
    \begin{scope}[shift={(6.45cm, -4.5cm)}]
        ...
        % allocations
        \foreach \ns/\nend/\strat/\v in  {  fs4/fs6/\kspff/7,
                                            fs48/fs50/\kspff/8,
    	                                      fs19/fs21/\kspff/9,
                                            fs23/fs25/\kspff/10,
                                            fs30/fs32/\kspff/11,
        ...
        % Text
        \foreach \heurtext/\v in  {   {\textbf{F}irst \textbf{F}it}/7,
    	                               {\textbf{L}ast \textbf{F}it}/8,
                                      {\textbf{B}est \textbf{F}it \textbf{F}irst}/9,
                                      {\textbf{B}est \textbf{F}it \textbf{L}ast}/10,
                                      {\textbf{W}orst \textbf{F}it \textbf{F}irst}/11,
                                      {\textbf{W}orst \textbf{F}it \textbf{L}ast}/12}{
    	...
    	\foreach \i in {1,...,\amount}{
    		\pgfmathtruncatemacro{\j}{\i-1}
    		\node[slot, minimum width = \slotwidth, draw = black!20, fill = \kspff!50, line width = 0.5*\lw, visible on = <6->] (fse\i) at (\j*\slotwidth+0.125*\lw+5cm, 2.4) {};
    	}

        % arrows
        \foreach \s/\v in { fs5/7,
                            fs20/9,
                            fs24/10,
                            fs31/11,
                            fs43/12,
