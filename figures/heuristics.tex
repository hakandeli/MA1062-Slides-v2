...
\begin{tikzpicture}
    ...
    \node[rectangle, fill = gray!40, minimum width = 14.7cm, minimum height = 6.5cm, rounded corners = 5pt, anchor = north west, visible on = <2->] at (0,0) {};
    ...
    \node[rectangle, fill = white, minimum width = 8cm, minimum height = 5cm, rounded corners = 5pt, anchor = north west, visible on = <2->] at (6.2,-0.8) {};

    %text
    \node[anchor = west, visible on = <2->] at (0.1, -0.35) {Policy $\pi_{\text{KSP-...}}$};
    \node[anchor = center, align = center, visible on = <2->] at (3.1, -6.15) {K-shortest paths (KSP)};
    \node[anchor = center, align = center, visible on = <2->] at (10.2, -6.15) {Heuristic slot selection};
    ...
    \begin{scope}[x=0.9cm, y= 0.6cm, shift={(-0.2cm,-1cm)}]
        \foreach \x/\y/\lbl/\i in { 1.5/-4/{1.}/0, 2/-4/A/1, 3/-4/B/2, 4/-4/F/3, 5/-4/E/4,%
                                    ...
                                    \node[visible on = <2->] (n\i) at (\x, \y) {\small \lbl};
                                }
    \end{scope}
    \foreach \i/\j in { 1/2, 2/3, 3/4, 
                        ...
                        \draw[->, > = Stealth, line width = 0.2pt, visible on = <2->] (n\i) -- (n\j);
    ...
    \begin{scope}[shift={(6.45cm,-3.3cm)}]
    
        %EON
    	\pgfmathtruncatemacro{\amount}{\numchannels*\fsperchannel}
    	\foreach \i in {1,...,\amount}{
    		\pgfmathtruncatemacro{\j}{\i-1}
    		\node[slot, minimum width = \slotwidth, draw = black!20, line width = 0.5*\lw, visible on = <2->] (fs\i) at (\j*\slotwidth+0.125*\lw, 0) {};
    	}
        ...
        \node[draw = black, 
              minimum width = 10*\slotwidth*\fsperchannel, 
    		   ...
    		   anchor = west, 
    		   line width = \lw,
    		   inner sep = 0mm, visible on = <2->] (startboxeon) at (-0.005, 0) {};

        \draw[->, line width = 1pt, draw = red, > = Stealth , shorten >= 0.5mm, visible on = <2->] ($(fs13.north) + (0, 0.6)$) -- (fs13.north);
