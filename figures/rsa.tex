...
\begin{tikzpicture}
    ...
    \begin{scope}[x=1.3cm, y=1.1cm, shift={(3.5cm, -2.6cm)}]
        ...
        \draw[edge, draw = green, ->, visible on = <4->, line width = 1pt] (nB) -- (nF);
        \draw[edge, draw = purple, ->, visible on = <4->, line width = 1pt] (nB) -- (nD);
        \draw[edge, draw = purple, ->, visible on = <4->, line width = 1pt] (nD) -- (nF);
    ...
    \begin{scope}[shift={(8cm,-2.5cm)}]
        ...
        % draw chosen spectrum

        \draw[->, line width = 1pt, draw = red, > = Stealth , shorten >= 0.5mm, visible on = <6->] ($(fs13.north) + (0, 0.6)$) -- (fs13.north);

      
    	\fill[fill=\kspff, opacity = 0.5, visible on = <6->] (fs13.north west)++(0.25*\lw,-0.7*\lw) -- ($(fs13.south west)+(0.25*\lw, 0.7*\lw)$) -- ($(fs16.south east) + (-0.25*\lw, 0.7*\lw)$) -- ($(fs16.north east)+(-0.25*\lw,-0.7*\lw)$) -- cycle;
    ...
    % constraints
    \node[rectangle, fill = IKRBlue!5, minimum width = 4.4cm, minimum height = 2cm, rounded corners = 5pt, anchor = north west, align = center, text height = 1cm, text depth = 0.5cm, visible on = <7->] at (0.2,-4.7) {enough FSs available};
